...
export class SubscriptionPlansComponent implements OnInit {
  ...
  planesDisponibles: Plan[] = [
    ...
    },
    {
      ...
      nombre: 'PROFESIONAL',
      descripcion:
        'Ideal para equipos en crecimiento con necesidades avanzadas',
      precio: '59.00',
      maximo_usuarios: 15,
      popular: true,
      caracteristicas: [
  ...
  getButtonText(plan: Plan): string {
    ...
    if (
      Number.parseFloat(plan.precio) > Number.parseFloat(this.planActual.precio)
    )
      return 'Mejorar Plan';
    return 'Cambiar Plan';
  }
  ...
  private procesarSuscripcion(
    ...
    this.subscriptionService.suscribirUsuario(request).subscribe({
      next: () => {
        this.mostrarExito(
          `¡Te has suscrito al plan ${plan.nombre} correctamente!`
        );
        this.obtenerPlanActual();
      },
      error: (error) => {
        const mensaje =
          error.error?.error || 'No se pudo completar la suscripción';
